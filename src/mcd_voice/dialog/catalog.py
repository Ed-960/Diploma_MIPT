"""Runtime-каталог блюд из Vector DB (Chroma) или из mcd.json для того же API."""
...
def _runtime_index_from_metadatas(
    metas: list[Any],
...
class MenuCatalog:
    """Загрузка меню из Chroma или из того же источника, что индексируется в Chroma (mcd.json)."""
    ...
    def load_runtime_index(
        ...
        return _runtime_index_from_metadatas(metas)

    def load_runtime_index_from_json(
        self,
        json_path: str | Path | None = None,
    ) -> tuple[list[str], dict[str, float], dict[str, list[str]], dict[str, dict[str, bool]]]:
        """Те же структуры, что ``load_runtime_index``, без подключения к Chroma/эмбеддингам."""
        _, _, metas = load_menu_from_json(json_path)
        return _runtime_index_from_metadatas(metas)
